import rclpy
import DR_init

# 로봇 설정 상수 (필요에 따라 수정)
ROBOT_ID = "dsr01"
ROBOT_MODEL = "m0609"
ROBOT_TCP = "Tool Weight" #본인 TCP 이름 설정
ROBOT_TOOL = "GripperDA_v1"  #본인 그리퍼 이름 설정

# 이동 속도 및 가속도 (필요에 따라 수정)
VELOCITY = 100
ACC = 60

# DR_init 설정
DR_init.__dsr__id = ROBOT_ID
DR_init.__dsr__model = ROBOT_MODEL


def initialize_robot():
    """로봇의 Tool과 TCP를 설정"""
    from DSR_ROBOT2 import set_tool, set_tcp  # 필요한 기능만 임포트

    # Tool과 TCP 설정
    set_tool(ROBOT_TCP)
    set_tcp(ROBOT_TOOL)

    # 설정된 설정값 출력
    # print("#"*50)
    print("Initializing robot with the following settings:")
    print(f"ROBOT_ID: {ROBOT_ID}")
    print(f"ROBOT_MODEL: {ROBOT_MODEL}")
    print(f"ROBOT_TCP: {ROBOT_TCP}")
    print(f"ROBOT_TOOL: {ROBOT_TOOL}")
    print(f"VELOCITY: {VELOCITY}")
    print(f"ACC: {ACC}")
    print("#"*50)


def perform_task():
    print("Performing task...")
    from DSR_ROBOT2 import posx, movej, movel, DR_MV_MOD_REL, DR_MV_MOD_ABS, set_digital_output, ON, OFF, DR_TOOL, DR_BASE, get_current_posx, wait

    def grip():
        set_digital_output(1, ON)
        set_digital_output(2, OFF)
        wait(0.5)

    def release():
        set_digital_output(1, OFF)
        set_digital_output(2, ON)
        wait(0.5)

    def down():
        movel([0, 0, -50, 0, 0, 0], v=VELOCITY, a=ACC, ref=DR_TOOL, mod=DR_MV_MOD_REL)
    
    def up():
        movel([0, 0, 50, 0, 0, 0], v=VELOCITY, a=ACC, ref=DR_TOOL, mod=DR_MV_MOD_REL)

    def home():
        JReady = [0, 0, 90, 0, 90, 0]
        movej(JReady, v=VELOCITY, a=ACC)

    def dig():
        digging = [0, 0, 30, 0, 0, 0]
        movel(digging, v=VELOCITY, a=ACC, ref=DR_TOOL, mod=DR_MV_MOD_REL)

    def move_to_dig():
        target = posx([-75, -200, 0, 0, 0, 0])
        movel(target, v=VELOCITY, a=ACC, ref=DR_BASE, mod=DR_MV_MOD_ABS)

    def tilt_down():
        movel([0, 0, 0, -45, 0, 0], v=VELOCITY, a=ACC, ref=DR_TOOL, mod=DR_MV_MOD_REL)

    def tilt_up():
        movel([0, 0, 0, 45, 0, 0], v=VELOCITY, a=ACC, ref=DR_TOOL, mod=DR_MV_MOD_REL)

    def get_curr_pose_base():
        p = get_current_posx()
        if isinstance(p, (list, tuple)):
            p_list = list(p[:6])
        else:
            p_list = list(p)
        return posx(p_list)

    home()
    release()

    move_to_tool_box = posx([150, -100, 0, 0, 0, 0])
    movel(move_to_tool_box, v=VELOCITY, a=ACC, ref=DR_BASE, mod=DR_MV_MOD_REL)

    down()
    grip()
    up()
    home()

    move_to_dig()

    def all_digging():
        for i in range(5):
            move_forward = [0, 100, 0, 0, 0, 0]

            a = get_current_posx()

            # a가 [x,y,z,rx,ry,rz] 같은 1차원 리스트/튜플인 경우
            if isinstance(a, (list, tuple)) and isinstance(a[0], (int, float)):
                curr = a[:6]

            # a가 ([x,y,z,rx,ry,rz], ref) 같은 2중 구조인 경우
            elif isinstance(a, (list, tuple)) and isinstance(a[0], (list, tuple)):
                curr = a[0][:6]

            # 혹시 다른 타입이면 그냥 리스트로 바꾸고 앞 6개만 사용
            else:
                curr = list(a)[:6]

            dig()
            movej([0,0,0,45,0,0], v = VELOCITY, a = ACC)  # ref, mod 없이
            up()
            movel([30,0,0,0,0,0], v = VELOCITY, a = ACC, ref = DR_BASE, mod = DR_MV_MOD_REL)
            movej([0,0,0,0,0,45], v = VELOCITY, a = ACC)
            movel(curr, v = VELOCITY, a = ACC, ref = DR_BASE, mod = DR_MV_MOD_ABS)
            movel(move_forward, v = VELOCITY, a = ACC, ref = DR_BASE, mod = DR_MV_MOD_REL)


    all_digging()

    movel([-100, -450, 0, 0, 0, 0], v=VELOCITY, a=ACC, ref=DR_BASE, mod=DR_MV_MOD_REL)

    all_digging()

        

def main(args=None):
    """메인 함수: ROS2 노드 초기화 및 동작 수행"""
    rclpy.init(args=args)
    node = rclpy.create_node("test", namespace=ROBOT_ID)

    # DR_init에 노드 설정
    DR_init.__dsr__node = node

    try:
        # 초기화는 한 번만 수행
        initialize_robot()

        # 작업 수행
        perform_task()

    except KeyboardInterrupt:
        print("\nNode interrupted by user. Shutting down...")
    except Exception as e:
        print(f"An unexpected error occurred: {e}")
    finally:
        rclpy.shutdown()

if __name__ == "__main__":
    main()